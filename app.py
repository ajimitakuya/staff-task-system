...
def get_knowbe_diary_saved_records(resident_id: str):
    """
    saved_documents から、Knowbe日誌下書きの保存済み一覧を取得する
    """
    resident_id = str(resident_id or "").strip()
    if not resident_id:
        return pd.DataFrame()

    df = get_document_records("knowbe日誌下書き", resident_id)
...
def get_latest_ic_card_id_from_bridge(bridge_id: str = "main_reader", max_age_seconds: int = 20):
    ...
    r = row.iloc[0]
    card_id = str(r.get("last_card_id", "")).strip()
...
def save_login_card_id_to_user(user_id: str, card_id: str):
    users_df = get_users_df()
    if users_df is None or users_df.empty:
        return False, "users シートが空です"

    users_df = users_df.fillna("").copy()

    if "login_card_id" not in users_df.columns:
        users_df["login_card_id"] = ""

    user_id = str(user_id).strip()
    card_id = str(card_id).strip().upper()

    if not user_id:
        return False, "user_id が空です"
    if not card_id:
        return False, "card_id が空です"

    # 既に他の人に使われていないか確認
    dup = users_df[
        (users_df["login_card_id"].astype(str).str.strip().str.upper() == card_id) &
        (users_df["user_id"].astype(str).str.strip() != user_id)
    ]
    if not dup.empty:
        dup_name = str(dup.iloc[0].get("display_name", dup.iloc[0].get("user_id", ""))).strip()
        return False, f"このカードIDはすでに別の利用者/職員に登録されています: {dup_name}"

    mask = users_df["user_id"].astype(str).str.strip() == user_id
    if not mask.any():
        return False, "対象 user_id が見つかりません"

    users_df.loc[mask, "login_card_id"] = card_id
    save_db(users_df, "users")
    return True, "保存しました"

def build_knowbe_diary_form_data(
    target_company_id,
    target_company_name,
    resident_id,
    resident_name,
    ...
    return {
        ...
        "company_name": str(target_company_name).strip(),
...
def apply_pending_bee_journal_load():
    """
    Knowbe日誌下書き読込用。
    widget生成前に session_state へ戻す。
    """
    pending_json = st.session_state.pop("bee_journal_pending_load_json", None)

    if not isinstance(pending_json, dict):
        return

    # 日付
    target_date_str = str(pending_json.get("target_date", "")).strip()
    if target_date_str:
        try:
            st.session_state["bee_target_date"] = pd.to_datetime(target_date_str).date()
        except Exception:
            pass

    # 基本入力
    st.session_state["start_time"] = str(pending_json.get("start_time", "")).strip()
    st.session_state["end_time"] = str(pending_json.get("end_time", "")).strip()
    st.session_state["bee_meal_flag"] = str(pending_json.get("meal_flag", "なし")).strip() or "なし"
    st.session_state["bee_service_type"] = str(pending_json.get("service_type", "在宅")).strip() or "在宅"

    st.session_state["bee_work_start_time"] = str(pending_json.get("work_start_time", "")).strip()
    st.session_state["bee_work_end_time"] = str(pending_json.get("work_end_time", "")).strip()
    st.session_state["bee_work_break_time"] = str(pending_json.get("work_break_time", "")).strip()

    st.session_state["bee_staff_name"] = str(pending_json.get("staff_name", "")).strip()

    # 備考
    note_mode = str(pending_json.get("note_mode", "候補から選ぶ")).strip()
    if note_mode not in ["候補から選ぶ", "直接入力"]:
        note_mode = "候補から選ぶ"
    st.session_state["bee_note_mode"] = note_mode

    note_text = str(pending_json.get("note_text", "")).strip()
...
def mark_bee_journal_generated_texts(generated_status: str, generated_support: str):
    st.session_state["bee_generated_status"] = str(generated_status or "").strip()
    st.session_state["bee_generated_support"] = str(generated_support or "").strip()
...
def clear_login_card_id_from_user(user_id: str):
    users_df = get_users_df()
    if users_df is None or users_df.empty:
        return False, "users シートが空です"
    ...
    users_df.loc[mask, "login_card_id"] = ""
    save_db(users_df, "users")
    return True, "ICカード登録を削除しました。"
...
def render_ic_card_manage_page():
    ...
    if staff_df is None or staff_df.empty:
        st.info("登録対象のスタッフがまだいません。")
        return

    # 念のため列補完
    if "login_card_id" not in staff_df.columns:
        staff_df["login_card_id"] = ""
    if "display_name" not in staff_df.columns:
        staff_df["display_name"] = ""
    if "user_login_id" not in staff_df.columns:
        staff_df["user_login_id"] = ""
    if "user_id" not in staff_df.columns:
        staff_df["user_id"] = ""
    ...
    with tab1:
        ...
        if unregistered.empty:
            ...
            with read_cols[0]:
                st.markdown("##### カード読取")
                if st.button("カードを読み取る", key="read_ic_card_new", use_container_width=True):
    ...
    with tab2:
        ...
        if registered.empty:
            ...
            for _, row in registered.iterrows():
                ...
                user_login_id = str(row.get("user_login_id", "")).strip()
                user_id = str(row.get("user_id", "")).strip()
                current_card = str(row.get("login_card_id", "")).strip()

                if user_login_id:
                    label = f"{display_name} ({user_login_id}) [現在: {current_card}]"
                else:
                    label = f"{display_name} [現在: {current_card}]"

                change_label_map[label] = user_id

            change_choices = ["スタッフを選択してください"] + list(change_label_map.keys())
            ...
            st.info("①スタッフを選ぶ → ②新しいICカードをタッチ → ③『カードを読み取る』 → ④『登録変更する』")

            change_cols = st.columns([1, 1])

            with change_cols[0]:
                st.markdown("##### カード読取")
                if st.button("カードを読み取る", key="read_ic_card_change", use_container_width=True):
                    if selected_change_label == "スタッフを選択してください":
                        st.error("スタッフを選択してからICカードを読み取ってください。")
                    else:
                        card_id, err = get_latest_ic_card_id_from_bridge("main_reader", 20)
            ...
            with change_cols[1]:
                st.markdown("##### 新しいカードID")
                change_card_id_input = st.text_input(
                    "新しいカードID",
                    key="ic_change_card_id",
                    label_visibility="collapsed",
                    placeholder="ここに新しいカードIDが自動入力されます",
                    help="通常はICカードをタッチして『カードを読み取る』で自動入力します。"
                )

            preview_change_card_id = str(st.session_state.get("ic_change_card_id", "")).strip()
            if preview_change_card_id:
                st.caption(f"現在の読取結果: {preview_change_card_id}")

            if st.button("登録変更する", key="ic_change_button", use_container_width=True):
                if selected_change_label == "スタッフを選択してください":
                    st.error("スタッフを選択してからICカードを変更してください。")
                else:
                    target_user_id = str(change_label_map.get(selected_change_label, "")).strip()
                    new_card_id = str(change_card_id_input or "").strip().upper()

                    if not target_user_id:
                        st.error("変更対象スタッフの user_id が取得できません。")
                    elif not new_card_id:
                        st.error("新しいカードIDが空です。ICカードをタッチして『カードを読み取る』を押してください。")
                    else:
                        ok, msg = save_login_card_id_to_user(target_user_id, new_card_id)
                        if ok:
                            create_admin_log(
                                action_type="ic_card_change",
                                target_type="user",
                                target_id=target_user_id,
                                action_detail=f"login_card_id={new_card_id}"
                            )
                            st.success(msg)
                            st.rerun()
                        else:
    ...
    with tab3:
        ...
        if registered.empty:
            ...
            for _, row in registered.iterrows():
                ...
                delete_label_map[label] = user_id

            delete_choices = ["スタッフを選択してください"] + list(delete_label_map.keys())
            ...
            st.warning("このスタッフのICカード登録を削除します。")

            if st.button("登録削除する", key="ic_delete_button", use_container_width=True):
                if selected_delete_label == "スタッフを選択してください":
                    st.error("スタッフを選択してからICカード登録を削除してください。")
                else:
                    target_user_id = str(delete_label_map.get(selected_delete_label, "")).strip()

                    if not target_user_id:
                        st.error("削除対象スタッフの user_id が取得できません。")
                    else:
                        ok, msg = clear_login_card_id_from_user(target_user_id)
                        if ok:
                            create_admin_log(
...
def render_assessment_form_page(doc_title: str):
    ...
    with housing_cols[1]:
        housing_status_other = st.text_input(
            ...
            disabled=(housing_status != "その他")
        )

    # 利用可能な交通手段
    transport_options = ["電車", "バス", "自転車", "徒歩", "その他"]
    ...
    with st.expander("入力内容確認"):
        st.write({
            ...
            "家族状況": family_rows,
            "住居の状況": [housing_status, housing_status_other],
            "利用可能な交通手段": [available_transport, available_transport_other, available_use_status, available_use_status_other],
            "生活歴": life_rows,
            "病名": disease_name,
            "症状": disease_symptom,
...
def render_bulk_knowbe_diary_page():
    ...
    login_staff_name = str(
        st.session_state.get("user", "") or st.session_state.get("display_name", "")
    ).strip()
    ...
    # ===== Knowbeログイン情報解決（キャッシュ） =====
    cached_company_id = str(st.session_state.get("bulk_bee_ctx_company_id", "")).strip()
    ctx = st.session_state.get("bulk_bee_ctx", {})

    if (not isinstance(ctx, dict)) or (not ctx) or cached_company_id != current_company_id:
        ...
        st.session_state["bulk_bee_ctx"] = ctx
        st.session_state["bulk_bee_ctx_company_id"] = current_company_id

    if not ctx.get("ok", False):
        st.error(ctx.get("error", "事業所情報の確認に失敗しました。"))
        return

    resolved_knowbe_user = str(ctx.get("knowbe_login_username", "")).strip()
    resolved_knowbe_pw = str(ctx.get("knowbe_login_password", "")).strip()
    ...
    if residents_df is None or residents_df.empty:
        st.warning("利用者データがありません。")
        return

    if "status" in residents_df.columns:
        residents_df = residents_df[
            residents_df["status"].fillna("利用中").astype(str).str.strip() == "利用中"
        ].copy()
    ...
    def _ensure_block_defaults(resident_id: str):
        block_key = _block_key(resident_id)

        defaults = {
            f"{block_key}_enabled": True,
            f"{block_key}_edit_mode": False,
            f"{block_key}_start_time": "10:00",
            f"{block_key}_end_time": "10:50",
            f"{block_key}_meal_flag": "なし",
            f"{block_key}_service_type": "在宅",
            f"{block_key}_work_start_time": "",
            f"{block_key}_work_end_time": "",
            f"{block_key}_break_time": "",
            f"{block_key}_staff_name": login_staff_name,
            f"{block_key}_remark_mode": "候補から選ぶ",
            f"{block_key}_remark_text": "",
            f"{block_key}_remark_text_select": "在宅利用",
            f"{block_key}_start_memo": "",
            f"{block_key}_end_memo": "",
            f"{block_key}_send_mode": "入力文のまま送信",
            f"{block_key}_generated_start_memo": "",
            f"{block_key}_generated_end_memo": "",
            f"{block_key}_piecework_id": "",
            f"{block_key}_piecework_name": "",
            f"{block_key}_piecework_quantity": "",
            f"{block_key}_loaded_record_id": "",
            f"{block_key}_saved_updated_at": "",
        }
    ...
    def _load_saved_entry_into_state(resident_id: str, saved_json: dict, record_id: str = "", updated_at: str = ""):
        if not isinstance(saved_json, dict):
            return
    ...
    def _save_single_bulk_entry(resident_id: str, resident_name: str):
        ...
        if loaded_record_id:
            ...
            saved_id = loaded_record_id
            created_new = False
        else:
            saved_id = save_document_record(
                resident_id=resident_id,
                resident_name=resident_name,
        ...
        st.session_state[f"{block_key}_loaded_record_id"] = saved_id
        st.session_state[f"{block_key}_saved_updated_at"] = now_jst().strftime("%Y-%m-%d %H:%M:%S")
        return saved_id, created_new

    def _get_latest_saved_record(resident_id: str):
        saved_df = get_knowbe_diary_saved_records(resident_id)
        if saved_df is None or saved_df.empty:
            return "", "", None

        try:
            if "updated_at" in saved_df.columns:
                saved_df = saved_df.sort_values("updated_at", ascending=False)
        except Exception:
            pass
        ...
        updated_at = str(row.get("updated_at", "")).strip()
        saved_json = load_document_json(record_id) if record_id else None
        return record_id, updated_at, saved_json
    ...
    for _, r in residents_df.iterrows():
        ...
        _ensure_block_defaults(resident_id)
        block_key = _block_key(resident_id)

        latest_record_id, latest_updated_at, latest_saved_json = _get_latest_saved_record(resident_id)

        # 初回だけ保存済み存在情報を表示用に持つ（自動読込はしない）
        if latest_updated_at and not str(st.session_state.get(f"{block_key}_saved_updated_at", "")).strip():
            st.session_state[f"{block_key}_saved_updated_at"] = latest_updated_at

        edit_mode = bool(st.session_state.get(f"{block_key}_edit_mode", False))
        inputs_disabled = not edit_mode

        with st.container():
            st.subheader(f"{resident_name}")

            saved_info = str(st.session_state.get(f"{block_key}_saved_updated_at", "")).strip()
            if saved_info:
                st.caption(f"保存済みあり: {saved_info}")
            else:
                st.caption("保存済みなし")
            ...
            with c3:
                current_meal = st.session_state.get(f"{block_key}_meal_flag", "なし")
                if current_meal not in ["なし", "あり"]:
                    current_meal = "なし"
                meal_flag = st.selectbox(
                    "食事提供",
                    ["なし", "あり"],
                    index=["なし", "あり"].index(current_meal),
            ...
            if remark_mode == "候補から選ぶ":
                ...
                default_remark = st.session_state.get(f"{block_key}_remark_text_select", "在宅利用")
                if default_remark not in remark_candidates:
                    default_remark = "在宅利用"
    ...
    if st.button("保存済みの利用者を一括送信", type="primary", use_container_width=True):
        enabled_residents = []
        for _, r in residents_df.iterrows():
            resident_id = str(r.get("resident_id", "")).strip()
            resident_name = str(r.get("resident_name", "")).strip()
            ...
            block_key = _block_key(resident_id)
            if bool(st.session_state.get(f"{block_key}_enabled", True)):
                enabled_residents.append({
                    "resident_id": resident_id,
                    "resident_name": resident_name,
                })

        if not enabled_residents:
            st.warning("送信対象の利用者がいません。")
            return

        send_targets = []
        invalid_messages = []

        for item in enabled_residents:
            resident_id = item["resident_id"]
            resident_name = item["resident_name"]

            latest_record_id, latest_updated_at, latest_saved_json = _get_latest_saved_record(resident_id)

            if not latest_record_id or not isinstance(latest_saved_json, dict):
                invalid_messages.append(f"{resident_name}：保存済み日誌がありません")
                continue

            # 対象日が違ってもそのまま使うのではなく、画面の対象日に更新して送る
            payload = dict(latest_saved_json)
            payload["target_date"] = bulk_target_date.strftime("%Y-%m-%d")
            payload["resident_id"] = resident_id
            payload["resident_name"] = resident_name

            staff_name = str(payload.get("staff_name", "")).strip()
            if not staff_name:
                invalid_messages.append(f"{resident_name}：日誌入力者が未入力")
                continue

            send_targets.append(payload)

        if invalid_messages:
            st.warning("送信できない利用者があります：")
        ...
        st.session_state["bulk_send_runtime_targets"] = send_targets
    ...
    if runtime_running:
        ...
        if runtime_stop_requested:
            ...
            st.session_state["bulk_send_running"] = False
            status_box.success(
                f"送信完了：成功 {success_count}件 / エラー {error_count}件"
            )
            ...
            try:
                start_memo_to_send = str(item.get("start_memo_generated", "")).strip() or str(item.get("start_memo_raw", item.get("start_memo", ""))).strip()
                end_memo_to_send = str(item.get("end_memo_generated", "")).strip() or str(item.get("end_memo_raw", item.get("end_memo", ""))).strip()

                send_mode = str(item.get("send_mode", "raw")).strip()
                if send_mode == "gemini":
                    start_memo_to_send = edit_text_with_gemini_for_start_memo(
                        resident_name=resident_name,
                        original_text=str(item.get("start_memo_raw", item.get("start_memo", ""))).strip(),
                        remark_text=str(item.get("note_text", item.get("remark_text", ""))).strip(),
                    )
                    end_memo_to_send = edit_text_with_gemini_for_end_memo(
                        resident_name=resident_name,
                        original_text=str(item.get("end_memo_raw", item.get("end_memo", ""))).strip(),
                        remark_text=str(item.get("note_text", item.get("remark_text", ""))).strip(),
                    )
                ...
                ok = send_to_knowbe_from_bee(
                    company_id=current_company_id,
                    target_date=str(item.get("target_date", bulk_target_date.strftime("%Y-%m-%d"))).strip(),
                    resident_name=resident_name,
                    service_type=str(item.get("service_type", "在宅")).strip() or "在宅",
                    start_time=str(item.get("start_time", "")).strip(),
                    end_time=str(item.get("end_time", "")).strip(),
                    meal_flag=str(item.get("meal_flag", "なし")).strip() or "なし",
                    note_text=str(item.get("note_text", item.get("remark_text", ""))).strip(),
                    generated_status=start_memo_to_send,
                    generated_support=end_memo_to_send,
                    staff_name=str(item.get("staff_name", "")).strip(),
                    knowbe_target="bulk_gemini" if send_mode == "gemini" else "bulk_raw",
                    work_start_time=str(item.get("work_start_time", "")).strip(),
                    work_end_time=str(item.get("work_end_time", "")).strip(),
                    work_break_time=str(item.get("work_break_time", item.get("break_time", ""))).strip(),
...
def render_bee_journal_page():
    ...
    load_msg = st.session_state.pop("bee_journal_load_message", "")
    if load_msg:
        st.success(load_msg)
    ...
    if not ctx:
        ...
        diary_df = get_knowbe_diary_saved_records(resident_id)
        ...
        if diary_df is not None and not diary_df.empty:
            try:
                if "updated_at" in diary_df.columns:
                    diary_df = diary_df.sort_values("updated_at", ascending=False)
            except Exception:
                pass

            for _, r in diary_df.head(20).iterrows():
                ...
                saved_start = str(saved_json.get("start_time", "")).strip()
                saved_end = str(saved_json.get("end_time", "")).strip()

                label = f"{saved_date} / {saved_start}〜{saved_end} / ID:{record_id}"
                if updated_at:
                    label += f" / {updated_at}"

                load_options.append(label)
                load_map[label] = record_id
        ...
        with load_col2:
            ...
            if st.button("呼び出す", key="bee_load_saved_record", use_container_width=True):
                selected_record_id = load_map.get(selected_saved_label)
        ...
        if bulk_send_gemini:
            try:
                ...
                mark_bee_journal_generated_texts(
                ...
                if ok:
                    ...
                    loaded_record_id = st.session_state.get("bee_journal_loaded_record_id")
                    if loaded_record_id:
                        update_document_record(loaded_record_id, auto_save_form_data)
                    else:
                        new_id = save_document_record(
                            resident_id=resident_id,
                            resident_name=resident_name,
                            doc_type="knowbe日誌下書き",
                            form_data=auto_save_form_data
                        )
                        st.session_state["bee_journal_loaded_record_id"] = new_id
